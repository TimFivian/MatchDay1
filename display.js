...
function renderPenalties(game) {
  ...
  game.penaltiesHome.forEach(p => {
    homeBox.innerHTML += `<div class="penalty">#${p.number} ${formatTime(p.time)}</div>`
  })

  game.penaltiesAway.forEach(p => {
    awayBox.innerHTML += `<div class="penalty">#${p.number} ${formatTime(p.time)}</div>`
  })
}
...
function renderPowerplay(game) {
  ...
  // Heim in Unterzahl → Gast im Powerplay
  if (game.penaltiesHome.length > 0 && game.penaltiesAway.length === 0) {
    homeBar.style.display = "none"
    awayBar.style.display = "block"
  }
  // Gast in Unterzahl → Heim im Powerplay
  else if (game.penaltiesAway.length > 0 && game.penaltiesHome.length === 0) {
...
function showTimeout(game) {
  ...
  if (game.timeout) {
    const team = game.timeout === "home" ? game.homeTeam : game.awayTeam
    el.textContent = "TIMEOUT — " + team
  }
  else if (game.breakTime > 0) {
    el.textContent = "PAUSE " + formatTime(game.breakTime)
  }
  else {
    el.textContent = ""
  }
...
updateDisplay()game.penalties.away.forEach(p=>{
